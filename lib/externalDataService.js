...
const API_URL_DOLAR_BLUE_PROXY = '/api/exchange';
...
/**
 * Obtiene la cotización del Dólar Blue (Compra) usando la ruta Proxy de Next.js para evitar CORS.
...
export async function getUSDExchangeRate() {
  ...
  try {
    // CAMBIO CLAVE: Usamos la ruta proxy interna /api/exchange
    const response = await fetch(API_URL_DOLAR_BLUE_PROXY, { next: { revalidate: 3600 } }); 
    ...
    if (blueRate && blueRate.compra) {
      ...
      return blueRate.compra; 
    }

    throw new Error('Could not find BLUE rate in the API response.');

  } catch (error) {
    console.error(`ERROR al obtener Dólar Blue (PROXY FALLÓ): ${error.message}`);

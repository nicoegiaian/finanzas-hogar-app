...
// ─── INVERSIONES ───────────────────────────────────────────────────────────
// Agregar estas funciones al final de lib/supabaseClient.js

export const fetchInversionesInstrumentos = () =>
  fetchTableRows('inversiones_instrumentos', 'created_at');

export const fetchInversionesMovimientos = () =>
  fetchTableRows('inversiones_movimientos', 'fecha');

export const fetchPreciosManuales = () =>
  fetchTableRows('inversiones_precios_manuales', 'updated_at');

export const createMovimiento = (payload) =>
  createTableRow('inversiones_movimientos', payload);

export const upsertPrecioManual = async (instrumento_id, precio) => {
  // Borra el precio anterior del instrumento y crea uno nuevo
  const deleteQuery = `?instrumento_id=eq.${encodeURIComponent(instrumento_id)}`;
  const deleteUrl = buildTableUrl('inversiones_precios_manuales', deleteQuery);
  await fetch(deleteUrl, {
    method: 'DELETE',
    headers: buildHeaders(),
    cache: 'no-store',
  });
  return createTableRow('inversiones_precios_manuales', {
    instrumento_id,
    precio,
...
export const createInversionInstrumento = (payload) =>
  createTableRow('inversiones_instrumentos', payload);
 
export const updateInversionInstrumento = (id, fields) =>
  updateTableRow('inversiones_instrumentos', 'id', id, fields);
